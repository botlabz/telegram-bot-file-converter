# Telegram Bot: File Converter — Free & Open Source 🤖

**File Converter** is a free, open-source Telegram bot that converts files between common formats directly through Telegram (documents, images, audio, video) via an optional conversion API. Self-host it for free on Cloudflare Workers — full source code included (MIT).
...
- Upload a file to convert it
- Configurable conversion endpoint
...
2. **Create the Worker**
   ...
   2. Name it `telegram-bot-file-converter` and click **Deploy** (you'll land in the code editor).
...
4. **Point Telegram at your bot** — open this URL in your browser (use your Worker's real URL, shown in the dashboard; `<subdomain>` is your account's `*.workers.dev` subdomain):
   ```
   https://telegram-bot-file-converter.<subdomain>.workers.dev/register
   ```
...
```bash
git clone https://github.com/botlabz/telegram-bot-file-converter.git
cd telegram-bot-file-converter
npm install
```
...
wrangler deploy      # live at https://telegram-bot-file-converter.<subdomain>.workers.dev
```
...
```text
https://telegram-bot-file-converter.<your-subdomain>.workers.dev/register
```
...
| --- | --- |
| `/(send file)` | Convert FILE to FILE |
| `/help` | Show help |
